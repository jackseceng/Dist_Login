...
def new_user():
    ...
    if keydrive is True:
        ...
        data_handler.store_key_fingerprint(new_username, key, fingerprint, 'a', 'new')
        ...
        input("KEYDRIVE not detected, insert KEYDRIVE into local server port then press enter to try again. (New accounts cannot be created over ssh)")
        new_user()
...
def user_sign_out(username, group):
    ...
    if group is 'a':
        ...
        fingerprint = crypto_handler.fingerprint_data(key)
        data_handler.store_key_fingerprint(username, key, fingerprint, group, 'exist')
